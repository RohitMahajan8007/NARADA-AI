    ...
    if (isNewTelegramId) {
      try {
        await import("../utils/telegram.util.js").then((m) => m.sendTelegramMessage(
          telegramId,
          `✅ <b>Telegram Integration Successful</b>\n\nHello ${updatedUser.fullname || updatedUser.name || 'User'}! You have successfully linked your Telegram account with your WebMonitor profile. You will now receive alerts here.`
        ));
      } catch (e) {
        console.error("Failed to send welcome telegram message", e);
      }
    }
