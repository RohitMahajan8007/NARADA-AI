  ...
  try {
    ...
    if (action === "approve") {
      ...
      if (user) {
        ...
        const subject = "Payment Approved - Subscription Active";
        const message = `Great news! Your payment of ₹${payment.finalAmount || payment.amount} (UTR: ${payment.utrNumber || payment.utr}) has been approved. Your ${payment.plan.toUpperCase()} plan is now active.`;
        
        if (user.notificationPreferences?.email) {
          try {
            await import("../utils/sendEmail.js").then((m) => m.sendEmail({
              email: user.email,
              subject,
              message,
              html: `<h3>Payment Approved</h3><p>Your payment of <b>₹${payment.finalAmount || payment.amount}</b> has been approved.</p><p>Your <b>${payment.plan.toUpperCase()}</b> plan is now active.</p>`
            }));
      ...
      if (user) {
        ...
        const message = `Unfortunately, your payment of ₹${payment.finalAmount || payment.amount} (UTR: ${payment.utrNumber || payment.utr}) was rejected. Please contact support if you think this is a mistake.`;
        ...
        if (user.notificationPreferences?.email) {
          try {
            await import("../utils/sendEmail.js").then((m) => m.sendEmail({
              ...
              subject,
              message,
              html: `<h3>Payment Rejected</h3><p>Your payment of <b>₹${payment.finalAmount || payment.amount}</b> has been rejected.</p><p>Please contact support for more details.</p>`
            }));
          } catch (e) { console.error("Email send error", e); }
        }
...
export const updateCoupon = async (req, res) => {
  try {
    const { id } = req.params;
    const { code, discountType, discountAmount, maxUses, expiryDate, isActive } = req.body;
