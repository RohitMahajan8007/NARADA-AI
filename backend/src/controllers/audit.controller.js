...
export const getAudit = async (req, res) => {
  try {
    ...
    if (audit) {
      ...
      if (diffInDays < 5) {
        console.log(`[AUDIT] Returning cached PageSpeed data for ${monitor.url} (${Math.floor(diffInDays)} days old)`);
        return res.status(200).json({ success: true, data: { audit, cached: true } });
      }
    }

    res.status(200).json({ success: true, data: { audit: null, message: "Audit expired or not found. Please run deep audit." } });
  } catch (error) {
